...
function drawSelectedPoints(projectName,datasetName) {
	// ** converted from tree to matrix for the occurences app. 
	// use the girder API to extract the matrix.  use the Girder API to find the collection, the
	// "Data" directory, and then the item from inside the Data folder by name
	        ...
	        girder.restRequest({path: "/collection?text="+projectName})
		      		  ...
		      		  .done(_.bind(function (itemList) {
		      		  	...
		      		  	for (var i = itemList.length - 1; i >= 0; i--) {
		      		  		if (itemList[i]['name'] == datasetName) {
		      		  			//console.log('found occurrence set!')
							...
							if (itemId != null) {
								...
								// get the data for each occurrence and add the point on the google map
								// set the icon according to the name field
								var pointindex = 0
					    		var tree_return_url = 'item/'+itemId+'/flow/table/csv/rows'
								girder.restRequest({path: tree_return_url})
					    			.done(_.bind(function (result) {
										phylomap.currentTable = result.data;
										for (var i = result.data.rows.length - 1; i >= 0; i--) {
											phylomap.selectedOccurrences.push(result.data.rows[i])
											var point = result.data.rows[i]
											var icon = getIcon(point['name']);
											var latlng = new google.maps.LatLng(
												parseFloat(point['lat']),
												parseFloat(point['long']));
											// start with the name at the top of the pop-up
											var text = '[name]:'+point['name'] + '\n'
											// add all attributes in the table to the pop-up
											for (var attrib in point) {
												text = text + '['+attrib +']:'+ '\t'+point[attrib].toString() + '\n'
											}
											var text = text + "location: " + latlng
											var name = point['name'];
											// assign a unique index to each marker, create an icon
											// according to the taxon name
											createMarker(latlng, name, text, pointindex, icon);
											pointindex++

										}
										//console.log("points found:",phylomap.selectedOccurrences)
	            ...
	            girder.restRequest({path: data_folder_url})
					...
					girder.restRequest({path: itemlist_url})
	            		...
	            		d3.select("#data").selectAll("option")
	            			.filter(function(d) { return (d["name"].indexOf("csv") > -1) })
	                		.data(itemList)
	                		.enter().append("option")
	               			.text(function (d) { return d.name; });
            			...
            			d3.select("#data").on("change", function () {
                    		...
                    		if (dataName ==  initialData) {
                    			console.log("draw:",projectName,dataName);
                    			updateJSON()
                    		} else {
                    			console.log('null')
                    		}
...
function initDatasetFromArbor() {
    ...
    phylomap.currentProjectName = collectionName;
    phylomap.currentDatasetName = dataName;
    phylomap.selectedOccurrences = [];
    //initializeDataSelection(phylomap.currentProjectName, phylomap.currentDatasetName)
    //updateTableDisplay(phylomap.selectedOccurrences);
    drawSelectedPoints(collectionName,dataName);
...
function off(d, callback) {
	// *** replace with code to clear the map
...
function loadOccurrences() {
	console.log('load occurrences')
	initDatasetFromArbor()
}
...
function updateJSON() {
	...
	var tree_return_url = 'item/'+phylomap.currentDatasetArchiveId+'/flow/table/csv/csv'
	girder.restRequest({path: tree_return_url})
		.done(_.bind(function (result) {
