...
phylomap.currentRows = null
phylomap.currentFields = null
...
function findSelectedOccurrences(projectName,datasetName) {
	...
	if (itemId != null) {
		...
		girder.restRequest({path: table_return_url})
			.done(_.bind(function (result) {
				...
				phylomap.currentRows = result.data.rows;
				phylomap.currentFields = result.data.fields;
				//console.log('fields: ',phylomap.currentFields)
				//console.log('rows: ',phylomap.currentRows)
		        ...
		        processOccurrencesForMapLocations()
				// initialize the display to show occurrences
				//update(root);
			}))
	}
}
	            ...
	            girder.restRequest({path: data_folder_url})
					...
					girder.restRequest({path: itemlist_url})
            			...
            			d3.select("#data").on("change", function () {
                    		...
                    		if (dataName.indexOf("csv") > -1) {
                    			findSelectedOccurrences(projectName,dataName);
                    		}
