...
// Assign icons according to the scientific name of the occurrence.  We accumulate a dictionary
// of indices and store them according to the scientific name, then when the name is passed, return
// either the same index or the next index. 
function getIcon(nodename) {
	if (typeof iconIndex === "undefined" || iconIndex == null || iconIndex == iconList.length) {
		iconIndex = 0;
		// save this as a species we have seen before, remember the index
		iconSpecies[nodename] = iconIndex
		markerColorIndex[nodename] = iconIndex
		iconIndex++
	} 
	if (nodename in iconSpecies) {
		return iconList[iconSpecies[nodename]]
	} else {
		iconSpecies[nodename] = iconIndex
		markerColorIndex[nodename] = iconIndex
		iconIndex++
...
// CRL: lookup the color according to the index set when points were created
...
function getIconColor(name) {
    ...
    if (typeof markerColorIndex[name] == "undefined") {
    	markerColorIndex[name] = name;
    }
    return markerColorIndex[name]
}
...
function createMarker(latlng, name, text, id, icon) {
	...
	//console.log('markerColorIndex:',markerColorIndex)
	//console.log('iconSpecies:',iconSpecies)
	//console.log('occurrences:',phylomap.selectedOccurrences)
	...
	if (typeof markerIndex[name] === "object") {
		markerIndex[name].push(markers.length);
	} else {
		markerIndex[name] = [markers.length];
	}
	markers.push(marker);
...
function updateOccurrencePointColors() {
    ...
    for (var i = 0; i < phylomap.selectedOccurrences.length; i++) {
 		...
 		// add/update the rendering color for each active point
 		//phylomap.selectedOccurrences[i]['renderColor'] = {r:redColor, g:greenColor, b:1}
 		phylomap.selectedOccurrences[i]['renderColor'] = redColor
    }
    ***/
    // redraw the map
