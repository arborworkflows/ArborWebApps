                ...
                if (result.status === 'SUCCESS') {
                    ...
                    girder.restRequest({path: result_url}).done(_.bind(function (data) {
                        ...
                        function getCol(matrix, col){
                         ...
                         var picx = getCol(app.pic.rows, "picX");
                         var picy = getCol(app.pic.rows, "picY")
                         ...
                         var slope = app.modelfit_summary.rows[0]["Slope"];

                         function getRegressionLine(xvar, slope, intercept) {
                           var regressionY = [];
                           for(var i=0; i<xvar.length; i++){
                             regressionY.push(intercept+slope*xvar[i]);
                           }
                           return(regressionY)
                         }

                         var predictedY = getRegressionLine(picx, slope, 0);
                         ...
                         var data = [contrasts, regressionLine];

                         var layout = {
                           title:'PIC plot',
                           xaxis: {
                             title: app.column_x + " PICs",
                             showgrid: true,
                             zeroline: true
                           },
                           yaxis: {
                             title: app.column_y + " PICs"
                           }
                         };

                         Plotly.newPlot('pic-plot', data, layout);
                         $('html, body').animate({
                             scrollTop: $("#pic-plot").offset().top
                         }, 1000);

                         $("#result").append("<h2>Results:<\h2>");
                         $("#result").append("<b>Column analyzed:<br> X (predictor) = <b>", app.column_x, "<br><b>Y (response) = <b>", app.column_y, "<br>");
                         $("#result").append("<b>PIC slope</b> = ", slope.toFixed(3), "<br>");
                         var pval=app.modelfit_summary.rows[0]["Pr(>F)"];
                         $("#result").append("<b>Significance test: </b>");
                         if(pval > 0.001) {
                           $("#result").append("P = ", pval.toFixed(3), "<br>");
                         } else {
                           $("#result").append("P < 0.001 <br>");
                         }
