...
function parseVegaSpec(spec, dynamicData) {
    ...
    vegaview = chart({
            el:"#tablecontent",
            data: {table: dynamicData.entries, fields:dynamicData.fields, namelist: dynamicData.namelist}
        })
        .update();
...
function update_table(tableRows,keyvalue) {
    "use strict";

    // a table rows object has two components: 'fields' and 'rows'.  'fields' is an array of strings, which are the 
    // column names.  The rows list is filled with dictionaries that use the fields values as keys to a dictionary

    // unroll the tableRows object into triples  (name, measurement, value) to match the vega spec
    //dynamData.fields = tableRows.fields
    var dynamData = {}
    dynamData.entries = []
    for (var row in tableRows.rows) {
        for (var attrib in tableRows.rows[row]) {
            if (attrib != keyvalue) {
                var record = {'name': tableRows.rows[row][keyvalue],'measurement': attrib,"value": tableRows.rows[row][attrib]}
                //console.log(record)
                dynamData.entries.push(record)
            }
        }
    }
    dynamData.namelist = []
    for (var  row in tableRows.rows) {
        dynamData.namelist.push(tableRows.rows[row][keyvalue])
    }
    dynamData.fields = tableRows.fields
